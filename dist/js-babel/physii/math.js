...
(function (p) {
  p.math = {
    ...
    closeTo: function closeTo(val1, val2, precision) {
      precision = precision || 0.0000001;
      return Math.abs(val1 - val2) < precision;
    },
    closeToArr: function closeToArr(arr1, arr2, precision) {
      var result = true;
      precision = precision || 0.0000001;
    ...
    wrapF: function wrapF(val, start, end) {
      var range = end - start;
    ...
    wrap: function wrap(val, start, end) {
      var range = end - start;
    ...
    inRanges: function inRanges(val, ranges) {
      var success = false;
    ...
    loneRanger: function loneRanger(val, inFrom, inTo, outFrom, outTo) {
    ...
    getLowestSolution: function getLowestSolution(solutions) {
      if (!solutions) {
        return;
      }
    ...
    quadraticFormula: function quadraticFormula(a, b, c) {
      var solutions = null;
      var toRoot = 0,
          root = 0;
    ...
    convMPerSec: function convMPerSec(val, to) {
      var returnVal = 0;
    ...
    closest: function closest(val, vals) {
     ...
     */
    pivotNumber: function pivotNumber(val, pivot) {
     ...
     */
    between: function between(val, start, end, exclusive) {
      if (end < start) {
        var temp = end;
        end = start;
        start = temp;
    ...
    putInRange: function putInRange(val, start, end) {
      var temp = null;
     ...
     */
    getLineSegment: function getLineSegment(segmentIndex, start, end, numSegments, safetyDist) {
     ...
     *
     */
    getLineSegmentRatio: function getLineSegmentRatio(segmentIndex, start, end, segments, safetyDist) {
    ...
    interpolate: function interpolate(start, end, ratio, clamp) {
